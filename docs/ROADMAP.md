...
- [ ] **Polls chart: tell more of the story (Wikipedia "race" view).** This is voter-facing polish, and the page is ephemeral (planned to be archived after polling day), so the value of shipping pre-7-May is much higher than the value of shipping into a static archive afterwards. Today's `PollsChart.tsx` on `/guide#polls` draws one straight-line-between-points series per party over a ~12-month window, with no per-poll dots, no smoothing, no 2021 baseline, no seat projection, and only a `constituency ↔ regional` toggle. Wikipedia's [opinion polling page](https://en.wikipedia.org/wiki/Opinion_polling_for_the_2026_Scottish_Parliament_election) gives the same data more story: each poll is a translucent dot, a smoothed trend line per party rides over the dot cloud, the 2021 result is a horizontal reference line, and a separate MRP seat-projection table answers the "what does that mean in seats" question. The user feedback that the current chart "only shows one layer" maps onto exactly that gap — we plot the trend without the volume, the dispersion, the 2021 anchor, or the seat consequence.

  Data side, two of the three Wikipedia tables are already ingested. `data/polls.json` already has separate `constituency` and `regional` arrays (135 polls each, refreshed daily by `scripts/sync-polls.ts`); the chart's existing toggle drives off these. What is *not* ingested is the third Wikipedia table — the MRP seat projections — which is what would let us show a "what does that mean in seats" panel below the chart. Extending `sync-polls.ts` to parse that table and persist a new additive `mrp:` array on `data/polls.json` is the only data work needed.

  Recommended scope is a single PR that keeps the hand-rolled SVG and adds three layers in `src/components/PollsChart.tsx`. First, per-poll dots (radius scaled by `sampleSize`, opacity ~0.4) drawn under the existing party lines so dispersion is visible at a glance. Second, a smoothed trend line per party computed via a centred 30-day rolling mean (pure function in a new `src/lib/polls-smoothing.ts`, ~30 lines, easy to unit-test against the existing fixtures); the existing straight-segment line drops to a thin "raw" overlay or is removed once the smoothed line is in. Third, a dashed horizontal reference line per party at its 2021 result (SNP 47.7% / 40.3%, etc., baselines from the same Wikipedia article) with a small label at the right edge.

  Replace the binary `constituency ↔ regional` toggle with a three-way "flip" select: **Constituency vote** / **Regional vote** / **Seat projection (MRP)**. The third option renders a small horizontal stacked bar per recent MRP, sorted newest-first, with a vertical "65 = majority" marker. Three or four most-recent rows is enough; the table is short. This addresses "only shows one layer" by making the layer choice explicit and by adding the seat-projection layer that the constituency-vs-regional toggle cannot show.

  Open decisions to settle in the PR description before coding. Time window: extend from ~12 months to the full 5 years from May 2021 to match Wikipedia's view, so the SNP-fall and Reform-rise are visible end-to-end. Smoothing window: 30-day centred rolling mean is the simplest defensible choice; LOESS would be nicer but is more code than the hand-rolled SVG ethos invites. Baseline visibility: on by default with a "show 2021 baselines" toggle to hide if visually overloading. The "flip" select uses the same UI pattern as the constituency↔region map view toggle (see `ConstituencyMap.tsx`'s segmented control) so it stays visually consistent. None of this needs a chart library.

  Files touched: `src/components/PollsChart.tsx` (chart layers, three-way flip, MRP panel), new `src/lib/polls-smoothing.ts` (pure rolling-mean), `scripts/sync-polls.ts` (third-table parser, additive `mrp:` field plus a `MIN_MRP_ROWS` guard so a Wikipedia rename also fails CI loudly), `data/polls.json` schema (add `mrp:` array), and a unit test alongside the existing parser tests. Out of scope: per-region polling trends (already deferred — Wikipedia's per-region breakdowns are too thin), pollster house-effect adjustment, and any chart-library swap. Single-sprint scope; the goal is to ship before 7 May 2026.
...
```
  ...
  18 Apr                                           7 May
    ...
    │  • Polls chart "race view" + 3-way flip         │
    │    (dots, smoothed line, MRP seat panel)        │
    │                                                 │
    │  Watch:                                         │
